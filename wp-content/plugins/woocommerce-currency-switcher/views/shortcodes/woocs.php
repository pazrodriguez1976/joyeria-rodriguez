	...
	if ( $drop_down_view == 'flags' ) {
		foreach ( $all_currencies as $key => $currency ) {
			...
			if ( ! empty( $currency['flag'] ) ) {
				?>
				<a href="#" class="woocs_flag_view_item 
				<?php
				if ( $this->current_currency == $key ) :
					?>
					woocs_flag_view_item_current<?php endif; ?>" data-currency="<?php echo esc_attr( $currency['name'] ); ?>" title="<?php echo esc_attr( $currency['name'] ) . ', ' . esc_attr( $currency['symbol'] ) . ' ' . esc_attr( $currency['description'] ); ?>"><img src="<?php echo esc_attr( $currency['flag'] ); ?>" alt="<?php echo esc_attr( $currency['name'] ) . ', ' . esc_attr( $currency['symbol'] ); ?>" /></a>
				<?php
			}
		}
	} else {
		...
		<form method="<?php echo esc_attr( apply_filters( 'woocs_form_method', 'post' ) ); ?>" action="" class="woocommerce-currency-switcher-form 
		<?php
		if ( $show_flags ) :
			?>
			woocs_show_flags<?php endif; ?>" data-ver="<?php echo esc_attr( WOOCS_VERSION ); ?>">
			...
			<select name="woocommerce-currency-switcher" data-width="<?php echo esc_attr( $width ); ?>" data-flag-position="<?php echo esc_attr( $flag_position ); ?>" class="woocommerce-currency-switcher <?php echo esc_attr( $css_class ); ?>" onchange="woocs_redirect(this.value); void(0);">
				<?php foreach ( $all_currencies as $key => $currency ) : ?>
					...
					<option class="woocs_option_img_<?php echo esc_attr( $key ); ?>" value="<?php echo esc_attr( $key ); ?>" <?php selected( $this->current_currency, $key ); ?> data-imagesrc="
					<?php
					if ( $show_flags ) {
						echo esc_attr( ( ! empty( $currency['flag'] ) ? $currency['flag'] : $empty_flag ) );}
					?>
					" data-icon="
					<?php
					if ( $show_flags ) {
						echo esc_attr( ( ! empty( $currency['flag'] ) ? $currency['flag'] : $empty_flag ) );}
					?>
" data-description="<?php echo esc_attr( $currency['description'] ); ?>"><?php echo esc_html( $option_txt ); ?></option>
				<?php endforeach; ?>
			</select>
	...
	if ( isset( $_GET['action'] ) and $_GET['action'] === 'elementor' ) {
		...
		?>
			<div data-woocs-sd='
		<?php
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- wc_esc_json() is a WooCommerce escaping function
		echo wc_esc_json( json_encode( $sd_settings ) )
		?>
			' data-woocs-ver='<?php echo esc_attr( WOOCS_VERSION ); ?>' style="width: <?php echo esc_attr( ( isset( $sd_settings['width'] ) ? esc_attr( $sd_settings['width'] ) . 'px' : 'auto' ) ); ?>; max-width: 100%;" data-woocs-sd-currencies='
			<?php
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- wc_esc_json() is a WooCommerce escaping function
			echo wc_esc_json( json_encode( $currencies ) )
			?>
			'><div class="woocs-lds-ellipsis"><div></div><div></div><div></div><div></div></div></div>
			<?php
	}
}
