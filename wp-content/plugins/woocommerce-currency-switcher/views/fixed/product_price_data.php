		...
		<li id="woocs_li_geo_<?php echo esc_attr( $hash ); ?>_<?php echo esc_attr( $index ); ?>">
			...
			<div class="variations-defaults woocs-variations-defaults">
				...
				<p class="form-row">
					...
					<select name="woocs_price_geo_countries[<?php echo esc_attr( $post_id ); ?>][<?php echo esc_attr( $index ); ?>][]" multiple="" size="1" 
					<?php
					if ( $index !== '__INDEX__' ) :
						?>
						class="woocs_price_geo_countries chosen_select woocs_country_select" 
						<?php
else :
	?>
						class="woocs_country_select" <?php endif; ?> data-placeholder="<?php esc_html_e( 'select some countries', 'woocommerce-currency-switcher' ); ?>">
...
<div class="woocs_multiple_simple_panel options_group pricing woocommerce_variation_fixed" style="
<?php
if ( $type == 'simple' ) :
	?>
