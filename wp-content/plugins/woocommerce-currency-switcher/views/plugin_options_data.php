...
function woocs_get_allowed_html_switcher23() {
...
function woocs_draw_switcher23( $name, $is_checked, $event = '', $label_title = '', $inversion = false ) {
	$id             = uniqid();
	$checked        = 'n';
	$selected_value = 0;
	...
	return '<div>' . woocs_draw_html_item(
		'input',
		array(
			'type'  => 'hidden',
			'name'  => $name,
...
function woocs_draw_html_item( $type, $data, $content = '' ) {
	$item = '<' . $type;
...
function woocs_draw_select( $data, $selected = '', $name = '', $id = '' ) {
	ob_start();
	?>
	<select class="woocs-form-select woocs-form-control" name="<?php echo esc_attr( $name ); ?>" 
	<?php
	if ( ! empty( $id ) ) :
		?>
...
function woocs_draw_select_e( $data, $selected = '', $name = '', $id = '' ) {
	?>
	<select class="woocs-form-select woocs-form-control" name="<?php echo esc_attr( $name ); ?>" 
	<?php
	if ( ! empty( $id ) ) :
